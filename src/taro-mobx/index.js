...
var mobx = require('mobx-for-taro');
var nervjs = require('nervjs');
console.log(mobx.configure);
...
function hoistNonNervStatics(targetComponent, sourceComponent, customStatics) {
  if (typeof sourceComponent !== 'string') {
    ...
    for (var i = 0; i < keys.length; ++i) {
      if (!NERV_STATICS[keys[i]] && !KNOWN_STATICS[keys[i]] && (!customStatics || !customStatics[keys[i]])) {
        try {
...
function isStateless(component) {
  return !(component.prototype && component.prototype.render) && !nervjs.Component.isPrototypeOf(component);
}
...
function createStoreInjector(grabStoresFn, component, injectNames) {
  ...
  function (_Component) {
    ...
    function Injector() {
      ...
      _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "storeRef", function (instance) {
    ...
    _createClass(Injector, [{
      ...
      value: function render() {
        ...
        return nervjs.createElement(component, newProps);
      }
    }]);

    return Injector;
  }(nervjs.Component); // Static fields from component should be visible on the generated Injector


  _defineProperty(Injector, "displayName", displayName);

  hoistNonNervStatics(Injector, component);
...
function findDOMNode(component) {
  if (nervjs.findDOMNode) {
    try {
      return nervjs.findDOMNode(component);
    } catch (e) {
      return null;
    }
  }
...
function makeComponentReactive(render) {
  ...
  var reaction = new mobx.Reaction("".concat(initialName, "#").concat(rootNodeID, ".render()"), function () {
    if (!isRenderingPending) {
      ...
      if (typeof _this2.componentWillReact === 'function') {
        _this2.componentWillReact();
      }
      ...
      if (_this2.__$mobxIsUnmounted !== true) {
        ...
        try {
          ...
          if (!_this2[skipRenderKey]) nervjs.Component.prototype.forceUpdate.call(_this2);
          hasError = false;
        } finally {
...
function observer(arg1, arg2) {
  ...
  if (componentClass.__proto__ === nervjs.PureComponent) {
  ...
  if (typeof componentClass === 'function' && (!componentClass.prototype || !componentClass.prototype.render) && !componentClass.isReactClass && !nervjs.Component.isPrototypeOf(componentClass)) {
    var _class, _temp;
    ...
    function (_Component) {
      ...
      return _class;
    }(nervjs.Component), _defineProperty(_class, "displayName", componentClass.displayName || componentClass.name), _defineProperty(_class, "contextTypes", componentClass.contextTypes), _defineProperty(_class, "propTypes", componentClass.propTypes), _defineProperty(_class, "defaultProps", componentClass.defaultProps), _temp));
    hoistNonNervStatics(observerComponent, componentClass);
...
var Observer = observer(function (_ref) {
  ...
  return nervjs.createElement(InjectComponent);
});
...
_defineProperty(Provider, "contextTypes", {
  mobxStores: objectOrObservableObject
});
...
if (!nervjs.Component) throw new Error('mobx-react requires React to be available');
if (!mobx.spy) throw new Error('mobx-react requires mobx to be available');
if (typeof nervjs.unstable_batchedUpdates === 'function') mobx.configure({
  reactionScheduler: nervjs.unstable_batchedUpdates
});
